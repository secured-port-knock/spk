...
// A full-strength 32-char base32 secret (20 bytes = 160 bits, as generated by setup).
const testTOTPSecret32 = "JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP"

func TestValidateTOTPSecretValid(t *testing.T) {
	// A 32-char base32 secret matches what GenerateTOTPSecret() produces
	// (20 bytes = 160 bits, base32-encoded without padding).
	cfg := DefaultServerConfig()
	cfg.TOTPEnabled = true
	cfg.TOTPSecret = testTOTPSecret32
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "totp_secret") {
			t.Errorf("valid 32-char TOTP secret should not produce error: %s", e)
		}
	}
}

func TestValidateTOTPSecret16CharsTooShort(t *testing.T) {
	// 16 chars = 80-bit secret -- below the RFC 6238 160-bit minimum.
	// Previously the validation incorrectly accepted this; it must now fail.
	cfg := DefaultServerConfig()
	cfg.TOTPEnabled = true
	cfg.TOTPSecret = "JBSWY3DPEHPK3PXP" // 16 chars
	errs := cfg.Validate()
	found := false
	for _, e := range errs {
		if strings.Contains(e, "totp_secret") {
	...
	if !found {
		t.Error("16-char TOTP secret (80-bit) should produce a validation error")
	}
}
...
func TestValidateTOTPSecretTooShort(t *testing.T) {
	// Any secret shorter than TOTPMinSecretLen (32) must be rejected.
	for _, short := range []string{"ABCD", "JBSWY3DPEHPK3PXP", "ABCDEFGHIJKLMNOPQRSTUVWXYZ23456"} {
		cfg := DefaultServerConfig()
		cfg.TOTPEnabled = true
		cfg.TOTPSecret = short
		errs := cfg.Validate()
		found := false
		for _, e := range errs {
		...
		if !found {
			t.Errorf("TOTP secret %q (%d chars) should produce a too-short error", short, len(short))
		}
	}
}

func TestValidateTOTPSecretExactlyMinLen(t *testing.T) {
	// Exactly 32 chars (the minimum) must pass.
	cfg := DefaultServerConfig()
	cfg.TOTPEnabled = true
	cfg.TOTPSecret = testTOTPSecret32 // exactly 32 chars
	if len(testTOTPSecret32) != TOTPMinSecretLen {
		t.Fatalf("test constant length %d, want %d", len(testTOTPSecret32), TOTPMinSecretLen)
	}
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "totp_secret") {
			t.Errorf("32-char secret at exact minimum should not produce error: %s", e)
		}
	}
}

func TestValidateTOTPSecretLongerThan32Valid(t *testing.T) {
	// Secrets longer than 32 chars are acceptable (extra entropy is fine).
	cfg := DefaultServerConfig()
	cfg.TOTPEnabled = true
	cfg.TOTPSecret = testTOTPSecret32 + "AAAA" // 36 chars, valid base32
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "too short") {
			t.Errorf("secret longer than minimum should not produce too-short error: %s", e)
		}
	}
}
...
// =============================================================================
// DynPortWindow validation tests
// =============================================================================

func TestValidateDynPortWindowZeroIsDefault(t *testing.T) {
	// 0 means "use the default (600)" -- must not produce an error.
	cfg := DefaultServerConfig()
	cfg.DynPortWindow = 0
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_window") {
			t.Errorf("DynPortWindow=0 (use default) should not produce error: %s", e)
		}
	}
}

func TestValidateDynPortWindowMinimumValid(t *testing.T) {
	// 60 is the documented minimum -- must be accepted.
	cfg := DefaultServerConfig()
	cfg.DynPortWindow = 60
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_window") {
			t.Errorf("DynPortWindow=60 (minimum) should not produce error: %s", e)
		}
	}
}

func TestValidateDynPortWindowMaximumValid(t *testing.T) {
	// 86400 is the documented maximum -- must be accepted.
	cfg := DefaultServerConfig()
	cfg.DynPortWindow = 86400
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_window") {
			t.Errorf("DynPortWindow=86400 (maximum) should not produce error: %s", e)
		}
	}
}

func TestValidateDynPortWindowTooSmall(t *testing.T) {
	// Below the minimum (60) must produce an error.
	for _, bad := range []int{-1, 1, 30, 59} {
		cfg := DefaultServerConfig()
		cfg.DynPortWindow = bad
		errs := cfg.Validate()
		found := false
		for _, e := range errs {
			if strings.Contains(e, "dynamic_port_window") {
		...
		if !found {
			t.Errorf("DynPortWindow=%d should produce a validation error", bad)
		}
	}
}

func TestValidateDynPortWindowTooLarge(t *testing.T) {
	// Above the maximum (86400) must produce an error.
	cfg := DefaultServerConfig()
	cfg.DynPortWindow = 86401
	errs := cfg.Validate()
	found := false
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_window") {
	...
	if !found {
		t.Errorf("DynPortWindow=86401 should produce a validation error")
	}
}

func TestValidateDynPortWindowCommonValues(t *testing.T) {
	// All typical documented values must be accepted without error.
	for _, ws := range []int{60, 120, 300, 600, 900, 1800, 3600, 86400} {
		cfg := DefaultServerConfig()
		cfg.DynPortWindow = ws
		errs := cfg.Validate()
		for _, e := range errs {
			if strings.Contains(e, "dynamic_port_window") {
				t.Errorf("DynPortWindow=%d should not produce error: %s", ws, e)
			}
		}
	}
}
...
// =============================================================================
// DynPortMin / DynPortMax port range validation tests
// =============================================================================

func TestValidateDynPortMinMaxValidDefaults(t *testing.T) {
	cfg := DefaultServerConfig()
	// Default values (10000, 65000) must pass without error.
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_min") || strings.Contains(e, "dynamic_port_max") {
			t.Errorf("default DynPortMin/Max should not produce error: %s", e)
		}
	}
}

func TestValidateDynPortMinMaxZeroIsDefault(t *testing.T) {
	// Both zero means "use defaults" and must be accepted.
	cfg := DefaultServerConfig()
	cfg.DynPortMin = 0
	cfg.DynPortMax = 0
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_min") || strings.Contains(e, "dynamic_port_max") {
			t.Errorf("DynPortMin=0,DynPortMax=0 should not produce error: %s", e)
		}
	}
}

func TestValidateDynPortMinNegative(t *testing.T) {
	cfg := DefaultServerConfig()
	cfg.DynPortMin = -1
	errs := cfg.Validate()
	found := false
	...
	if !found {
		t.Error("negative DynPortMin should produce a validation error")
	}
}

func TestValidateDynPortMaxNegative(t *testing.T) {
	cfg := DefaultServerConfig()
	cfg.DynPortMax = -500
	errs := cfg.Validate()
	found := false
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_max") {
			found = true
		}
	}
	if !found {
		t.Error("negative DynPortMax should produce a validation error")
	}
}

func TestValidateDynPortMinTooLarge(t *testing.T) {
	cfg := DefaultServerConfig()
	cfg.DynPortMin = 65536
	errs := cfg.Validate()
	found := false
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_min") {
	...
	if !found {
		t.Error("DynPortMin > 65535 should produce a validation error")
	}
}

func TestValidateDynPortMaxTooLarge(t *testing.T) {
	cfg := DefaultServerConfig()
	cfg.DynPortMax = 99999
	errs := cfg.Validate()
	found := false
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_max") {
	...
	if !found {
		t.Error("DynPortMax > 65535 should produce a validation error")
	}
}

func TestValidateDynPortMinMaxOrdering(t *testing.T) {
	// min == max is invalid (requires open range).
	cfg := DefaultServerConfig()
	cfg.DynPortMin = 40000
	cfg.DynPortMax = 40000
	errs := cfg.Validate()
	found := false
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_min") {
	...
	if !found {
		t.Error("DynPortMin == DynPortMax should produce a validation error")
	}
}

func TestValidateDynPortMinMaxValidCustomRange(t *testing.T) {
	// A wide range within valid port numbers must be accepted.
	cfg := DefaultServerConfig()
	cfg.DynPortMin = 20000
	cfg.DynPortMax = 50000
	errs := cfg.Validate()
	for _, e := range errs {
		if strings.Contains(e, "dynamic_port_min") || strings.Contains(e, "dynamic_port_max") {
			t.Errorf("valid custom range should not produce error: %s", e)
		}
	}
}
...
func TestValidateDynPortWindowNegative(t *testing.T) {
	...
	errs := cfg.Validate()
	if !containsSubstring(errs, "dynamic_port_window") {
