...
func RunSetup() {
	...
	wizardKeyStorage(reader, cfg)

	bundle, bundleFile := wizardImportBundle(reader)
	wizardSavePublicKey(cfg, bundle)
	applyBundleConfig(cfg, bundle)

	fmt.Println()
	wizardServerHost(reader, cfg)
...
// wizardKeyStorage prompts the user to select a key storage mode and adjusts
// cfg.KeyStorageMode. If the credential manager is chosen but unavailable,
// the wizard retries once and falls back to plaintext on failure.
func wizardKeyStorage(reader *bufio.Reader, cfg *config.Config) {
	...
	case 1:
		applyCredentialManagerStorage(reader, cfg)
	default:
		cfg.KeyStorageMode = "file"
	}
	fmt.Println()
...
// applyCredentialManagerStorage tests credential manager availability and sets
// cfg.KeyStorageMode. Retries once on failure, then falls back to "file".
func applyCredentialManagerStorage(reader *bufio.Reader, cfg *config.Config) {
...
// wizardImportBundle searches for an existing activation bundle file then
// prompts for a base64 bundle if none was found or the user declines.
// Returns the parsed bundle and the path of the bundle file found (if any).
func wizardImportBundle(reader *bufio.Reader) (*crypto.ExportBundle, string) {
...
// wizardSavePublicKey validates the public key from the bundle, encodes it as
// PEM and saves it according to cfg.KeyStorageMode. Exits on fatal errors.
func wizardSavePublicKey(cfg *config.Config, bundle *crypto.ExportBundle) {
	...
	if _, err := crypto.LoadPublicKeyBytes(ekBytes); err != nil {
		kemLabel := "ML-KEM-1024"
		if bundle.KEMSize == 768 {
			kemLabel = "ML-KEM-768"
		}
...
// applyBundleConfig copies server parameters from the parsed bundle into cfg.
func applyBundleConfig(cfg *config.Config, bundle *crypto.ExportBundle) {
	if bundle.Port > 0 {
...
// wizardServerHost prompts for the server host/IP address and stores it in cfg.
func wizardServerHost(reader *bufio.Reader, cfg *config.Config) {
